...
begin
    ...
    when not matched then
        insert values (s.flight_key, s.tail_number, s.flight_number,  s.cancelled, s.diverted, s.origin_airport, s.destination_airport,s.CUSTOMER_ID, s.the_year, 
        s.the_month, s.the_day,
        s.db_source);
end;
/
...
begin
    ...
    update set
        ...
        FLIGHT_NUMBER = cmt.FLIGHT_NUMBER,
        THE_YEAR = cmt.THE_YEAR,
        THE_MONTH = cmt.THE_MONTH,
        THE_DAY = cmt.THE_DAY,
		DB_SOURCE = cmt.db_source
    when not matched then
    insert values (cmt.complaint_key, cmt.complaint_id, cmt.complaint_type, cmt.complaint_description, cmt.complaint_status, cmt.compensation_amnt, 
    cmt.allocated_to, cmt.CUSTOMER_ID, cmt.FLIGHT_NUMBER, cmt.THE_YEAR, cmt.THE_MONTH, cmt.THE_DAY, cmt.db_source);
end;
/
